...
class CustomerForm(forms.ModelForm):
    class Meta:
        model = Customer
        fields = ['c_phone', 'c_fname','c_lname','c_address']
        widgets = {
'c_phone': forms.TextInput(attrs={
    'class': 'form-control', 
    'id': 'c_phone', 
    'data-val': 'true', 
    'data-val-required': 'Please enter Phone Number', 
    'placeholder': 'Enter Phone Number', 
    'pattern': '^\+?[0-9\s\-]+$',  # Accepts digits, optional '+' at the start, spaces, and dashes
    'title': 'Phone number should contain only digits, spaces, dashes, and may start with +'
}),
